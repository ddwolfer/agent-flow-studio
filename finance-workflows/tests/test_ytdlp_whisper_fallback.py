"""Regression tests for the captions→whisper fallback in _get_full_transcript.

Bug being guarded: a caption-fetch exception (YouTube rate-limit) used to abort
the whole try block, so the audio fallback never ran. These tests prove that a
captions failure now falls through to the local whisper transcriber."""
...
def test_captions_exception_falls_through_to_whisper(monkeypatch):
    m = _load()
...
def test_no_captions_uses_whisper(monkeypatch):
    m = _load()
    m._TRANSCRIPT_CACHE.clear()
...
def test_both_fail_reports_both_errors(monkeypatch):
    ...
    assert "whisper: model load failed" in r["error"]


def test_whisper_disabled_only_captions_error(monkeypatch):
    m = _load()
    m._TRANSCRIPT_CACHE.clear()
    def boom(url, langs): raise RuntimeError("429")
    monkeypatch.setattr(m, "_fetch_captions", boom)
    monkeypatch.setattr(m, "_asr", None)  # whisper not installed
