...
## 3.5 美股題材(2026-07-20 加入 — user 提議)

B 版(數據偵探)的選題池從 BTC/ETH 擴大為 **BTC/ETH ⊕ 美股**。差異化護城河:
廣場寫 BTC 者眾,寫「美股 SMC 結構 + 幣圈視角 + 總經」者近乎無;我們有 FRED +
美股 zone(deep-research 每天現成 JSON 可重用)+ 加密三線。幣安可交易代幣化美股,
$標籤返佣吃得到。

**節奏 = 內容驅動,非硬性一天幣一天股**(延續「發當天最有戲」原則):
- 當天美股有熱題/財報/結構事件 → B 寫美股(可重用當日 deep-research zone,或臨時
  對熱題個股跑 compute_zones)
- 當天加密是主戲 → B 寫幣
- A 版維持以 BTC/ETH 為主(信譽鏈建立在幣的關鍵位回扣上)

**美股 watchlist(預設):** 跟 deep-research 重疊(NVDA/TSLA/MU/MSFT/GOOG)+ 當日
廣場熱題個股臨時加。

**⚠️ 代幣化美股 $標籤紀律(硬規則):** 幣安代幣化美股標籤格式不統一(觀察到
$NVDAB / $NVDA.US / $GOOGL.US 多種)。**發文前必須驗證該股在幣安的正確可交易
$標籤字串**(查廣場同日貼文或平台);**驗不到就降級** —— 用 `#美股名`(hashtag)
或純文字提及,**絕不掛未驗證的 $標籤**(掛錯 = 讀者點了無法交易 = 返佣落空 + 外行)。
美股價位/結構一律來自 compute_zones 的股票 zone JSON,不猜。
