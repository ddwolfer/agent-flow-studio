...
def test_run_announcements_alerts_new_promo_then_dedups(monkeypatch, cfg, tmp_path):
    ann = {"annId": "A1", "annTitle": "USDGO 補貼活動", "annDesc": "...", "annUrl": "http://x"}
    monkeypatch.setattr(run_mod.announcements, "fetch_bitget", lambda **kw: ([ann], []))
...
def test_run_announcements_skips_non_promo(monkeypatch, cfg, tmp_path):
    ann = {"annId": "B1", "annTitle": "系統維護公告", "annDesc": "...", "annUrl": "u"}
    monkeypatch.setattr(run_mod.announcements, "fetch_bitget", lambda **kw: ([ann], []))
